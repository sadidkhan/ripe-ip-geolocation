...
from domain.models import Measurement, PingResult


class MeasurementRepository:
    """Handles measurement data persistence to CSV."""
    
    def __init__(self, measurements_csv: Path, failed_csv: Path, results_csv: Path):
        self.measurements_csv = measurements_csv
        self.failed_csv = failed_csv
        self.results_csv = results_csv
    
    def read_all_measurements(self) -> dict[str, int]:
        """Read all measurements from CSV. Returns dict of {target: measurement_id}."""
        if not os.path.exists(self.measurements_csv):
            return {}
        
        measurements = {}
        with open(self.measurements_csv, mode='r', newline='', encoding='utf-8') as file:
    ...
    def write_measurement(self, measurement: Measurement) -> None:
        ...
        os.makedirs(os.path.dirname(self.measurements_csv), exist_ok=True)
        new_file = not os.path.exists(self.measurements_csv)
        
        with open(self.measurements_csv, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if new_file:
    ...
    def read_fetched_results(self) -> set[int]:
    ...
    def write_ping_results(self, results: list[PingResult]) -> None:
        """Write ping results to CSV."""
        if not results:
            return
        
        os.makedirs(os.path.dirname(self.results_csv), exist_ok=True)
        file_exists = os.path.isfile(self.results_csv)
        ...
        with open(self.results_csv, "a" if file_exists else "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
